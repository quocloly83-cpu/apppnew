      ...
      try {
        ...
        for (const entry of entries) {
          ...
          html +=
            '<div class="item">' +
            '<div><b>Key:</b> ' + k + '</div>' +
            '<div><b>Lượt thiết bị còn:</b> ' + v.uses + '</div>' +
            '<div><b>Đã dùng:</b> ' + (v.device || "Chưa khóa") + '</div>' +
...
app.post("/api/check", (req, res) => {
  ...
  if (typeof item.totalDevices !== "number") item.totalDevices = item.devices.length + Number(item.usesLeft || 0);
  if (typeof item.usesLeft !== "number") item.usesLeft = Number(item.usesLeft || 0);
...
app.get("/api/list", (req, res) => {
  ...
  return res.json({
    ok: true,
    items: Object.entries(keys).map(([key, value]) => {
      if (!Array.isArray(value.devices)) value.devices = [];
      if (typeof value.totalDevices !== "number") value.totalDevices = value.devices.length + Number(value.usesLeft || 0);
      if (typeof value.usesLeft !== "number") value.usesLeft = Number(value.usesLeft || 0);
