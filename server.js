...
const ADMIN_KEY = process.env.ADMIN_KEY || "nguyentanhuyvip10thanhngannek";
const SESSION_SECRET =
  process.env.SESSION_SECRET || "ath_super_secret_huy_fanta_2026_fixed";
...
function normalizeKeyItem(item) {
  ...
  const oldUses = Number(item.uses || 0);
  const oldUsesLeft = Number(item.usesLeft || 0);

  if (typeof item.usesLeft !== "number") {
    item.usesLeft = oldUsesLeft || oldUses || 0;
  }
  if (typeof item.totalDevices !== "number") {
    item.totalDevices = Math.max(item.devices.length, item.devices.length + item.usesLeft);
  }
...
function renderLogo(size) {
  if (hasLogo()) {
    return `<img src="/logo.png" alt="AimTrickHead Logo" style="width:${size}px;height:${size}px;object-fit:cover;display:block">`;
  }
  return `<div style="width:${size}px;height:${size}px;display:flex;align-items:center;justify-content:center;font-size:${Math.round(
    size * 0.4
  )}px;color:#fff;background:linear-gradient(135deg,#8c52ff,#ff70c7)">⚡</div>`;
}
...
<body>
  <div class="splash" id="splash">
    <div class="splashLogo">${renderLogo(170)}</div>
  ...
  <div class="wrap">
    <div class="card">
      <div class="top">
        <div class="brand">
          <div class="logo">${renderLogo(72)}</div>
          <div>
      ...
      <div class="content">
        ...
        <div id="panelView" class="hidden">
          ...
          <div class="footer" id="deviceStats"></div>
  ...
  <script>
    ...
    function renderStats(data) {
      deviceStats.textContent = "Lượt còn: " + (data.usesLeft ?? "--") + " | Đã dùng: " + (data.usedDevices ?? 0) + "/" + (data.totalDevices ?? 0);
    }
    ...
    async function taoKey() {
      ...
      try {
        ...
        result.innerHTML =
          ...
          '<div>📱 Số thiết bị: ' + data.totalDevices + '</div>' +
      ...
      try {
        ...
        for (const v of data.items) {
          html +=
            ...
            '<div><b>Lượt còn:</b> ' + v.usesLeft + '</div>' +
...
app.get("/api/list", (req, res) => {
  ...
  const items = Object.entries(keys).map(([key, raw]) => {
    const v = normalizeKeyItem(raw);
    return {
      key,
      usesLeft: v.usesLeft,
      usedDevices: v.devices.length,
      totalDevices: v.totalDevices,
      expireAt: v.expireAt,
      expireText: formatVNTime(v.expireAt)
    };
  });
  saveStore();
